...
TEST_F(ParserTest, ParserHandlesStructMembers) {
  ...
  EXPECT_EQ(members->at(L"x").type, TypeIdentifier(L"int"));

  ASSERT_TRUE(members->find(L"y") != members->end());
  EXPECT_EQ(members->at(L"y").name, Identifier(L"y"));
  EXPECT_EQ(members->at(L"y").type, TypeIdentifier(L"int"));

  ASSERT_TRUE(members->find(L"z") != members->end());
  EXPECT_EQ(members->at(L"z").name, Identifier(L"z"));
  EXPECT_EQ(members->at(L"z").type, TypeIdentifier(L"int"));
}
